            ...
            $.ajax({ 
                ...
                success: function (response) {
                    $("#loadingIndicator").hide();
                        ...
                        if(response.GetNoDiePckgResult.length <=2 ){alert('Design ID not found');}
                        ...
                        $.each(JSON.parse(jsonData), function(k, v) {
                            ...
                            $('#DieInPackage').attr('size',8);   //changing the size attribute will change the dropdown to a listbox (hence auto expand)
                        });
                },  
                ...
                success: function (response) {
                    $("#loadingIndicator").hide();
                        ...
                        $.each(JSON.parse(jsonData), function(k, v) {
                            ...
                            var averageVal = DryWeightVal/count;
                            $("#resultBox").val(averageVal.toFixed(3));
                        });   
                },  
                error: function (jqXhr, statusText, errorThrown) { 
                    alert("readyState: " + jqXhr.readyState + "responseText: " + jqXhr.responseText + "StatusCode: " + jqXhr.status + "statusText: " +     jqXhr.statusText);
